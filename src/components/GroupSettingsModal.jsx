...
const ROLE_COLORS = { owner: '#f59e0b', admin: '#a78bfa', member: 'var(--text-muted)' }
...
function Avatar({ user, size = 42 }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: user?.avatar_url ? 'transparent' : color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: size * 0.38, fontWeight: 700, overflow: 'hidden', flexShrink: 0, position: 'relative' }}>
...
export default function GroupSettingsModal({ chat, session, myRole, onClose, onUpdated, showToast, onViewUser }) {
  ...
  async function searchUsers(q) {
    ...
    const { data } = await supabase.from('profiles').select('id, full_name, username, avatar_url, online')
      .neq('id', session.user.id).or(`username.ilike.%${q}%,full_name.ilike.%${q}%`).limit(8)
    const ids = members.map(m => m.id)
    setSearchResults(data?.filter(u => !ids.includes(u.id)) || [])
  }
  ...
  async function changeRole(userId, newRole) {
    ...
    await loadMembers()
    setRoleMenu(null)
    showToast(`Роль изменена: ${ROLE_LABELS[newRole]}`)
  }
  ...
  return (
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) { onClose(); setRoleMenu(null) } }}>
      <div className="modal" style={{ maxWidth: 460 }}>
        <div className="modal-title">⚙️ Настройки группы<button className="modal-close" onClick={onClose}>×</button></div>
        ...
        {tab === 'info' && (
          <div>
            <label htmlFor="grp-avatar-inp">
              <div style={{ width: 86, height: 86, borderRadius: '50%', background: chat.avatar_url ? 'transparent' : 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, fontWeight: 700, margin: '0 auto 16px', cursor: canManage ? 'pointer' : 'default', overflow: 'hidden', border: '2px dashed var(--accent)', position: 'relative' }}>
                {chat.avatar_url ? <img src={chat.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (chat.name || 'G')[0].toUpperCase()}
            ...
            {canManage && <input id="grp-avatar-inp" type="file" accept="image/*" style={{ display: 'none' }} onChange={changeGroupAvatar} />}

            <p style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: 13, marginBottom: 20 }}>{members.length} участников</p>
            ...
            {canManage ? (
              ...
              <p style={{ textAlign: 'center', fontSize: 18, fontWeight: 700, marginBottom: 20 }}>{chat.name}</p>
            )}

            <button onClick={() => removeMember(session.user.id)} style={{ width: '100%', marginTop: 12, padding: 12, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, color: '#fca5a5', cursor: 'pointer', fontSize: 14, fontWeight: 500 }}>
              🚪 Покинуть группу
            </button>
          </div>
        )}
        ...
        {tab === 'members' && (
          <div onClick={() => setRoleMenu(null)}>
            {members.map(member => (
              <div key={member.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--border)', cursor: 'pointer' }}
                onClick={e => { if (e.target.closest('button')) return; if (onViewUser && member.id !== session.user.id) onViewUser(member) }}>
                <Avatar user={member} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: ROLE_COLORS[member.role] }}>{ROLE_LABELS[member.role]}</div>
                </div>
                ...
                {canManage && member.id !== session.user.id && member.role !== 'owner' && (
                  <div style={{ display: 'flex', gap: 6, position: 'relative' }} onClick={e => e.stopPropagation()}>
                    ...
                    {roleMenu === member.id && (
                      <div style={{ position: 'absolute', right: 0, top: '110%', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', zIndex: 50, minWidth: 170, boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
        ...
        {tab === 'add' && canManage && (
          <div>
            <input className="form-input" placeholder="Поиск по имени или @username" value={search} onChange={e => searchUsers(e.target.value)} style={{ marginBottom: 12 }} />
            {searchResults.length === 0 && search.length >= 2 && <p style={{ color: 'var(--text-muted)', fontSize: 14, textAlign: 'center', padding: '20px 0' }}>Никого не найдено</p>}
