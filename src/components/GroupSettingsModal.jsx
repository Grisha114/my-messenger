...
import { Avatar, formatLastSeen, senderColor } from './helpers.jsx'

const ROLE_L={owner:'👑 Владелец',admin:'⭐ Админ',member:'👤 Участник'}
const ROLE_C={owner:'#f59e0b',admin:'#a78bfa',member:'var(--text3)'}

export default function GroupSettingsModal({ chat, session, myRole, onClose, onUpdated, onViewUser, showToast }) {
  ...
  async function searchUsers(v) {
    ...
    const {data} = await supabase.from('profiles').select('id,full_name,username,avatar_url,online,last_seen').neq('id',session.user.id).or(`username.ilike.%${v}%,full_name.ilike.%${v}%`).limit(8)
  ...
  async function saveInfo() {
    ...
    const {data} = await supabase.from('chats').update({name:gName.trim(),description:gDesc.trim()||null}).eq('id',chat.id).select().single()
  ...
  async function clearMyHistory() {
    if(!window.confirm('Очистить свою историю в этой группе?')) return
  ...
  async function deleteGroup() {
    ...
    if(!window.confirm('Удалить группу полностью? Необратимо.')) return
      ...
      <div className="modal wide" style={{maxWidth:500}}>
        ...
        <div className="grp-banner">
          <div className="grp-stat">
            ...
            <div className="grp-stat-item"><div className="grp-stat-num">{stats.media}</div><div className="grp-stat-label">Медиа</div></div>
          </div>
        </div>
            ...
            {canManage?(
              <>
                ...
                <div className="f-group">
                  <label className="f-label">Описание группы</label>
                  <textarea className="f-input" value={gDesc} onChange={e=>setGDesc(e.target.value)} rows={3} placeholder="О чём эта группа..."/>
                </div>
                <button className="btn-primary" onClick={saveInfo} disabled={loading||!gName.trim()}>{loading?'Сохранение...':'Сохранить'}</button>
              </>
            ):(
              <div style={{textAlign:'center',marginTop:16}}>
                <p style={{fontSize:20,fontWeight:700,marginBottom:6}}>{chat.name}</p>
                {chat.description&&<p style={{fontSize:14,color:'var(--text2)'}}>{chat.description}</p>}
              </div>
            )}

            {chat.description&&canManage&&<p style={{fontSize:13,color:'var(--text3)',marginTop:4}}>(описание видно всем участникам)</p>}

            <div style={{marginTop:20,display:'flex',flexDirection:'column',gap:8}}>
              <button onClick={clearMyHistory} style={{width:'100%',padding:12,background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:10,color:'var(--text2)',cursor:'pointer',fontSize:14,textAlign:'left'}}>
                🧹 Очистить мою историю
              </button>
              <button onClick={()=>removeMember(session.user.id)} style={{width:'100%',padding:12,background:'rgba(239,68,68,.08)',border:'1px solid rgba(239,68,68,.2)',borderRadius:10,color:'#fca5a5',cursor:'pointer',fontSize:14,textAlign:'left'}}>
                🚪 Покинуть группу
              </button>
              {myRole==='owner'&&(
                <button onClick={deleteGroup} style={{width:'100%',padding:12,background:'rgba(239,68,68,.15)',border:'1px solid rgba(239,68,68,.3)',borderRadius:10,color:'#fca5a5',cursor:'pointer',fontSize:14,textAlign:'left',fontWeight:600}}>
        ...
        {tab==='members'&&(
          <div onClick={()=>setRoleMenu(null)}>
            {members.map(m=>{
              const reallyOnline = m.online && m.last_seen && (Date.now()-new Date(m.last_seen))<3*60*1000
              return (
                <div key={m.id} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:'1px solid var(--border)',cursor:'pointer'}}
                  ...
                  <Avatar name={m.full_name} url={m.avatar_url} size={44} online={reallyOnline}/>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:12,display:'flex',gap:8,alignItems:'center'}}>
                      <span style={{color:ROLE_C[m.role]}}>{ROLE_L[m.role]}</span>
                      <span style={{color:reallyOnline?'var(--green)':'var(--text3)',fontSize:11}}>{formatLastSeen(m.last_seen,reallyOnline)}</span>
                    </div>
                  </div>
        ...
        {tab==='add'&&canManage&&(
          <div>
            ...
            {res.map(u=>{
              const ro=u.online&&u.last_seen&&(Date.now()-new Date(u.last_seen))<3*60*1000
              return (
                <div key={u.id} style={{display:'flex',alignItems:'center',gap:12,padding:'8px 0',borderBottom:'1px solid var(--border)'}}>
                  <Avatar name={u.full_name} url={u.avatar_url} size={40} online={ro}/>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:600,fontSize:14}}>{u.full_name}</div>
                    <div style={{fontSize:12,color:'var(--text3)'}}>@{u.username} · {formatLastSeen(u.last_seen,ro)}</div>
                  </div>
